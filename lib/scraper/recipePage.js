...
module.exports = function recipePage(spec) {
    var getPage = function getPage() {
        ...
        getTime = function getTime() {
            return getArticle().then(function (article) {
                var time = normaliseText(article.
                        find(".recipe-quick-summary").
                        text()),
                    startPos = time.search(/: /) + 2;
                return time.substring(startPos, Math.max(time.search(/ \|/), startPos)) ||
                    undefined;
            });
        },
                    ...
                    return lodash.
                        ...
                        reduce(function (groups, text) {
                            if (testText(text, /^extra materiaal:/i)) {
                                groups.push(createGroup("utensils"));
                            }
                            if (testText(text, /^om erbij te serveren:/i)) {
                                groups.push(createGroup("sides"));
                            }
        ...
        getRecipe = function getRecipe() {
            return q.
                ...
                spread(function (name, tags, time, introduction, price, ingredients, instructions) {
                    var recipe = {};
                    recipe.name = name;
                    recipe.tags = tags;
                    if (time) {
                    ...
                    if (introduction.utensils) {
                        recipe.utensils = introduction.utensils;
                    }
                    if (introduction.sides) {
                        recipe.sides = introduction.sides;
                    }
